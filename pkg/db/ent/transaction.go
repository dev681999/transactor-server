...
type TransactionEdges struct {
	...
	// OperationType holds the value of the operation_type edge.
...
// OperationTypeOrErr returns the OperationType value or an error if the edge
// was not loaded in eager-loading, or loaded but was not found.
func (e TransactionEdges) OperationTypeOrErr() (*OperationType, error) {
	if e.OperationType != nil {
		return e.OperationType, nil
	} else if e.loadedTypes[1] {
		return nil, &NotFoundError{label: operationtype.Label}
	}
	return nil, &NotLoadedError{edge: "operation_type"}
}
...
func (*Transaction) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case transaction.FieldCreateTime, transaction.FieldUpdateTime, transaction.FieldTimestamp:
...
func (t *Transaction) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case transaction.FieldOperationTypeID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field operation_type_id", values[i])
			} else if value.Valid {
				t.OperationTypeID = int(value.Int64)
			}
...
// QueryOperationType queries the "operation_type" edge of the Transaction entity.
func (t *Transaction) QueryOperationType() *OperationTypeQuery {
	return NewTransactionClient(t.config).QueryOperationType(t)
}
