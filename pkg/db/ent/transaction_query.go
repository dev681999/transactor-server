...
type TransactionQuery struct {
	...
	withOperationType *OperationTypeQuery
...
// QueryOperationType chains the current query on the "operation_type" edge.
func (tq *TransactionQuery) QueryOperationType() *OperationTypeQuery {
	query := (&OperationTypeClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withOperationType: tq.withOperationType.Clone(),
...
// WithOperationType tells the query-builder to eager-load the nodes that are connected to
// the "operation_type" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithOperationType(opts ...func(*OperationTypeQuery)) *TransactionQuery {
	query := (&OperationTypeClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withOperationType = query
	return tq
}
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withOperationType; query != nil {
		if err := tq.loadOperationType(ctx, query, nodes, nil,
			func(n *Transaction, e *OperationType) { n.Edges.OperationType = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadOperationType(ctx context.Context, query *OperationTypeQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *OperationType)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		fk := nodes[i].OperationTypeID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(operationtype.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "operation_type_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withOperationType != nil {
			_spec.Node.AddColumnOnce(transaction.FieldOperationTypeID)
		}
	}
